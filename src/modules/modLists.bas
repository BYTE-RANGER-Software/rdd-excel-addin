...
' -----------------------------------------------------------------------------------
Public Function GetNamedOrHeaderValue(ByVal wks As Worksheet, ByVal localName As String, ByVal headerArray As Variant) As String
    ...
    namedValue = wks.Range(localName).value
    On Error GoTo ErrHandler
    ...
    Set searchRange = wks.Range(wks.Cells(1, 1), wks.Cells(50, 20))
...
CleanExit:
    Exit Function
ErrHandler:
    MsgBox "Error (" & Err.Number & "): " & Err.Description, vbCritical, "GetNamedOrHeaderValue"
    Resume CleanExit
End Function
...
' -----------------------------------------------------------------------------------
Public Sub CollectColumnValues(ByVal wks As Worksheet, ByVal headerArray As Variant, ByVal valuesDict As Scripting.Dictionary)
    On Error GoTo ErrHandler
    ...
    Set searchRange = wks.Range(wks.Cells(1, 1), wks.Cells(50, 50))
    ...
    Set columnRange = wks.Range( _
        wks.Cells(headerRow + 1, foundColumnIndex), _
        wks.Cells(wks.Rows.Count, foundColumnIndex) _
    )
...
CleanExit:
    Exit Sub
ErrHandler:
    MsgBox "Error (" & Err.Number & "): " & Err.Description, vbCritical, "CollectColumnValues"
    Resume CleanExit
End Sub
...
CleanExit:
    Exit Sub
ErrHandler:
    MsgBox "Error (" & Err.Number & "): " & Err.Description, vbCritical, "CollectColumnValuesFromRange"
    Resume CleanExit
End Sub
    ...
    For categoryIndex = LBound(categoryHeadersArray) To UBound(categoryHeadersArray)
        ...
        For colIndex = categoryColumnOffset To categoryColumnOffset + columnsPerCategory - 1
            ...
            For rowIndex = headerRow + 1 To endRow
                cellValue = Trim$(CStr(wks.Cells(rowIndex, colIndex).value))
...
CleanExit:
    Exit Sub
ErrHandler:
    MsgBox "Error (" & Err.Number & "): " & Err.Description, vbCritical, "CollectColumnBlockGroupValues"
    Resume CleanExit
End Sub

' -----------------------------------------------------------------------------------
' Procedure : CollectNamedRangeValues
' Summary   : Collects all non-empty values from a named range on a worksheet
'             and stores them as unique keys in a dictionary.
'
' Parameters:
'   wks          [Worksheet] - The worksheet containing the named range.
...
' -----------------------------------------------------------------------------------
Public Sub CollectNamedRangeValues(ByVal wks As Worksheet, rangeName As String, ByVal valuesDict As Scripting.Dictionary)
    On Error GoTo ErrHandler
    ...
    On Error Resume Next
    Set rngNamed = wks.Range(rangeName)
    On Error GoTo ErrHandler
...
CleanExit:
    Exit Sub
ErrHandler:
    MsgBox "Error (" & Err.Number & "): " & Err.Description, vbCritical, "CollectNamedRangeValues"
    Resume CleanExit
End Sub
...
CleanExit:
    Exit Sub
ErrHandler:
    MsgBox "Error (" & Err.Number & "): " & Err.Description, vbCritical, "CollectTableColumnValues"
    Resume CleanExit
End Sub
...
' -----------------------------------------------------------------------------------
Public Sub UpdateNamedListRange(ByVal rangeName As String, ByVal wks As Worksheet, ByVal columnIdx As Long)
    On Error GoTo ErrHandler
    ...
    lastRow = wks.Cells(wks.Rows.Count, columnIdx).End(xlUp).Row
    If lastRow < 2 Then lastRow = 2

    reference = "='" & wks.Name & "'!" & wks.Range(wks.Cells(2, columnIdx), wks.Cells(lastRow, columnIdx)).Address
...
' -----------------------------------------------------------------------------------
Public Sub WriteDictSetToColumn(ByVal wks As Worksheet, ByVal valuesDict As Scripting.Dictionary, ByVal startRow As Long, ByVal columnIdx As Long, Optional ByVal writeValuesToNextColumn As Boolean = False)
    On Error GoTo ErrHandler
    ...
    For index = 1 To sortedKeys.Count
        wks.Cells(startRow + index - 1, columnIdx).value = sortedKeys(index)
        If writeValuesToNextColumn Then
            wks.Cells(startRow + index - 1, columnIdx + 1).value = valuesDict(sortedKeys(index))
        End If
    Next index
    ...
    For Each currentKey In newKeysDict.Keys
        ...
        If Len(valueText) > 0 Then
            If Not existingKeysDict.Exists(valueText) Then
                wks.Cells(nextRow, columnIdx).value = valueText
...
' Summary   : Returns the DataBodyRange of a table column for a given table and
'             column name on a worksheet.
'
' Parameters:
'   wks          [Worksheet] - Worksheet containing the table.
'   tableName    [String]    - Name of the ListObject.
'   columnName   [String]    - Name of the column within the table.
'
' Returns   :
'   Range - The DataBodyRange of the requested column, or Nothing if the table
'           or column cannot be resolved.
'
' Notes     :
'   - Uses On Error Resume Next intentionally to return Nothing instead of
'     raising an error when the table/column is not found.
' -----------------------------------------------------------------------------------
Public Function GetTableColumnRange(ByVal wks As Worksheet, ByVal tableName As String, ByVal columnName As String) As Range
    On Error Resume Next
    Set GetTableColumnRange = wks.ListObjects(tableName).ListColumns(columnName).DataBodyRange
    On Error GoTo 0
End Function

' -----------------------------------------------------------------------------------
' Function  : ClearTableColumn
' Summary   : Clears the contents of a structured table column.
'
' Parameters:
...
' Summary   : Internal helper to collect non-empty values from a column-like range
